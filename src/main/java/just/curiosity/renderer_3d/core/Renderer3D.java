...
public final class Renderer3D {
  ...
  private final Vector3D rotateOffsetVector;
...

  {
    ...
    rotateOffsetVector = new Vector3D(0, 0, 0);
    ...
    switch (keyboard.getCurrentKeyCode()) {
      ...
      case KeyEvent.VK_CONTROL -> {
        ...
        if (mouse.getWheelRotation() == 1) {
          scale /= Const.SCALE_FACTOR; // zoom out
        } else if (mouse.getWheelRotation() == -1) {
          scale *= Const.SCALE_FACTOR; // zoom in
        }
      }

      case KeyEvent.VK_SHIFT -> {
        if (mouse.isDragged()) {
          rotateOffsetVector.x += (rotateVector.x = mouse.getDiffX());
        }
      }

      default -> {
        if (mouse.isDragged()) {
          rotateOffsetVector.y += (rotateVector.y = mouse.getDiffY());
          rotateOffsetVector.z += (rotateVector.z = mouse.getDiffX());
        }
      }
    }
  ...
  public void onFrame(Runnable onFrame) {
  ...
  public Vector3D getRotateOffsetVector() {
    return rotateOffsetVector;
  }
